...
class TableMaker:
    ...
    def __init__(self, fontsize=14, encode_base64=True, limit_crop=True, for_document=True, background_color="#f5f5f5"):
    ...
    def parse_into_rows(self, html):
        ...
        num_cols = sum(int(el.get('colspan', 1))
                       for el in soup.find('tr').find_all(['td', 'th']))
    ...
    def print_table(self):
        ...
        for i, (yd, row) in enumerate(zip(row_locs, self.rows)):
            ...
            p = mpatches.Rectangle((x0, y), width=total_width, height=yd, fill=True,
                                   color=self.background_color, transform=self.fig.transFigure)
